(() => {
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\gallery.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\color.ts
  var colorsCache = {};
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\menu.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\createElement.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\colorScheme.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\scrollspy.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\smoothAnchors.ts
  ...
  // ns-hugo:C:\Users\yhl19\Desktop\loli\assets\ts\imageScheme.ts
  var BackgroundImageScheme = class {
    // 存储用户选择模式的键名，用于 localStorage
    localStorageKey = "BackgroundImageScheme";
    // 存储当前图片模式（image 或 no-image）
    currentScheme;
    // 构造函数，用于初始化模式切换按钮和当前模式
    constructor(toggleEl) {
      this.currentScheme = this.getSavedScheme();
      if (toggleEl) {
        this.bindClick(toggleEl);
    ...
    // 将当前模式保存到 localStorage，以便下次加载页面时应用
    saveScheme() {
      localStorage.setItem(this.localStorageKey, this.currentScheme);
    }
    // 绑定切换按钮的点击事件
    bindClick(toggleEl) {
      toggleEl.addEventListener("click", () => {
        this.currentScheme = this.currentScheme === "image" ? "no-image" : "image";
        this.setBodyClass();
        this.saveScheme();
      });
    }
    // 设置网页根元素的 `data-image` 属性，用于根据模式调整显示样式
    setBodyClass() {
      document.documentElement.dataset.image = this.currentScheme;
    }
    // 从 localStorage 获取用户上次选择的图片模式，没有则默认使用 'image' 模式
    getSavedScheme() {
      const savedScheme = localStorage.getItem(this.localStorageKey);
      return savedScheme === "image" || savedScheme === "no-image" ? savedScheme : "image";
    }
  };
  var imageScheme_default = BackgroundImageScheme;
  ...
  var Stack = {
    init: () => {
      ...
      new imageScheme_default(document.getElementById("background-mode-switch"));
    }
  };
